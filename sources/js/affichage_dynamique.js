...
var bouton_panier_affichage_infos_modal = document.getElementById(
  "ajout_panier_détail_bd"
);
...
function creer_session_storage() {
  ...
  sessionStorage.setItem(
    "classic",
    "[" + tableau_storage_category_classic + "]"
  );
...
function afficher_BD_random(tableau_category, nombre_affichage) {
  ...
  document
    .getElementById("body_page")
    .appendChild(div_container_affichage_accueil);
  ...
  for (let i = 0; i < nombre_affichage; i++) {
    ...
    card_bouton_panier.setAttribute(
      "class",
      "btn btn-primary bouton_panier_card"
    );
    card_bouton_panier.innerHTML =
      '<img class="logo img-fluid" src="assets/panier.svg" alt="">';
...
function afficher_details_BD(id_session_storage) {
  ...
  col_serie_BD_grande.textContent =
    "série : " + series.get(objet_album.idSerie).nom;
  ...
  col_auteur_BD_grande.textContent =
    "auteur : " + auteurs.get(objet_album.idAuteur).nom;
  ...
  col_resume_BD_grande.textContent =
  ...
  document
    .getElementById(id_session_storage)
    .appendChild(col_bouton_panier_BD_grande);
}
